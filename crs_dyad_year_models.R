...
crs_reshape_dy <- function(crs_data, ch_codes, dempurposecodes){
  ...
  crs_data2 <- crs_data %>%
    group_by(Year,DonorCode,RecipientCode,ch_type) %>%
    mutate(
    ...
    ungroup() %>%
    group_by(Year,DonorCode,RecipientCode) %>%
    select(Year,DonorCode,DonorName,RecipientCode,RecipientName,n,total_disbursement,gov_d_total,gov_r_total,
...
crs_by_types <- crs_reshape_dy(x,channel_codes,dempurposecodes)
...
for (i in dep.vars){
  form <- as.formula(paste(i, paste(c(ind.vars_all_controls,paste0('lag(',i,')')),collapse = '+'),sep = '~'))
  model <- plm(form, model = 'within', data = working, index = c('DonorCode','RecipientCode'))
...
stargazer(mod_polity2,mod_v2x_polyarchy,mod_v2x_libdem,mod_v2x_partipdem,mod_v2x_delibdem,mod_v2x_egaldem,
          mod_e_fh_cl,mod_e_fh_pair,mod_e_fh_pr,mod_e_fh_rol,mod_e_fh_status, type = 'text', out = './dyadic_all_ivs_controls_all_lag.txt')
